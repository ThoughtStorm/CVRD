...
function(lang, array, aspect, Deferred, cookie, json, topic, dojoScript, esriNS, esriConfig,
  esriRequest, esriUrlUtils, IdentityManager, OAuthInfo, portalUrlUtils, jimuUtils, arcgisPortal, registry, query) {
  /*jshint -W069 */
  ...
  var mo = {
    ...
    registerOAuthInfo: function(portalUrl, appId){
      ...
      if(!oAuthInfo){
		//BG - changed the oauth return url to VF page 
		//so that we can use that page as the redirect uri on ArcGIS online web app
		//because the path to jimu.js/oauth-callback.html changes everytime we load the page
        var oauthReturnUrl = window.location.protocol + "//" + window.location.host + "/apex/GISOauthCallBack";
		...
		//BG - calling the getCredential method to set the oAuthPopupConfirmation option to false
		//oAuthPopupConfirmation - displays a confirmation dialog before opening the oauth popup
