...
function(declare, lang, array, html, _WidgetBase, topic, on, query, domConstruct, domGeometry,
  ...
  MapManager, utils, OnScreenWidgetIcon, WidgetPlaceholder, LoadingShelter, tokenUtils) {
  ...
  clazz = declare([_WidgetBase], {
    ...
    postCreate: function(){
	  ...
	  //BG subscribe the token
	  dojo.subscribe('bgSecurityToken', lang.hitch(this, this._getSecurityToken));
    },
    ...
    _loadMap: function() {
      ...
      if(this.appConfig.tokenInfo && this.appConfig.tokenInfo.tokenServicesUrl && this.appConfig.tokenInfo.tokenServicesUrl != '' && this.appConfig.tokenInfo.serviceUrl && this.appConfig.tokenInfo.serviceUrl != ''){
		...
		this.bgManager.getToken();
		this._renewSecurityToken();
	  } else {
	...
	_getSecurityToken: function(tokenObj){		
		 // Following call in Token Utils sets up the cookie		
     tokenUtils.registerAuth2Hash(this._generateTokenObjHelper(tokenObj));		
    ...
    renewSecurityToken: function(){		
		var renewToken = setInterval(lang.hitch(this, function(){		
			this.bgManager.getToken();		
		}),this.appConfig.tokenInfo.renewTimer);		
	},		
			
	_generateTokenObjHelper: function(value){		
		var tokenObj = {		
							"access_token":value.token,		
							"expires_in": value.expires * 60, //converion from minutes to seconds		
							"server":this.appConfig.tokenInfo.serviceUrl,		
							"ssl": value.ssl,		
    ...
    _loadPreloadWidgets: function(appConfig) {
      ...
      all(defs).then(lang.hitch(this, function(){
		...
		//BG try to open Search widget here
		if(zoomOnLoad == 'true'){
			this.preloadWidgetIcons[4].onClick(); // 4 = eSearch widget
		}
		//BG try to open Search widget here
		else if(showDirections == 'true'){
			this.preloadWidgetIcons[3].onClick(); // 3 = Directions widget
		}
        this._doPostLoad();
