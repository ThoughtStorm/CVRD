  ...
  function(
    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
	  ...
	  startup: function() {
		...
		//this.bgManager.getAllUsersLayerList();
	  ...
	  _onBGSearchFinish: function(resultsToProject){
		...
		if(resultsToProject.length > 1){
			...
			// this is to get the optimized route by using the Directions Widget (id stored in this.directionWidgetId)
			if(this.isBGRouting)
			{
				WidgetManager.getInstance().triggerWidgetOpen(this.directionWidgetId).then(function(directionWidget) {
	  ...
	  _addBGLayer: function(rawResults, projectedPts){
		...
		for(var i = 1; i < results.length; i++){
			...
			// check if user is in Lightning experience. Action links are formatted differently in LEX vs SF Classic
		...
		_getDirectionWidgetId: function() {
			var widgetArr = WidgetManager.getInstance().appConfig.widgetOnScreen.widgets;
			var id = '';
			widgetArr.forEach(function(widget){
				if (widget.amdFolder === 'widgets/Directions/') {
					id = widget.id;
				}
			});
			this.directionWidgetId = id;
		},
