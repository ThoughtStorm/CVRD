...
//            console.info("spatial layer index: " + this.spatialLayerIndex);
	  ...
	  getBGUsers: function(results) {
		...
		} else if(zoomOnLoad == 'true' && zoomObjects != null)
		{
			...
			html.setStyle(this.divResultMessage,'display','none');
...
//        console.info(fldName, layerIndex);
      ...
      search: function (geometry, layerIndex, /* optional */ expressIndex, theValue, spatialRelationship, closeOnComplete) {
        ...
        if (geometry) {
		  ...
		  var layerValue = this.selectLayerGraphical.get('value');      // graphical search layer
		  // BG Graphic search
		  ...
		  var layerValue = this.selectLayerAttribute.get('value');      // text search layer
...
//modify the currentFeatures with the new results
        ...
        for (var i = 1; i < len; i++) {
          ...
          for (var key in results[i].obj) {
            if (key != 'Id' && key != 'RecordTypeId' && key != 'attributes') {
              ...
              templateString += this.resultFormatString.replace('[attribname]', val).replace('[attribvalue]', results[i].obj[key]);
              label += this.resultFormatString.replace('[attribname]', val).replace('[attribvalue]', results[i].obj[key]);
            }
          }
          templateString += "<br/>";
          // add action links
      ...
      bufferSearchInSearchWidget:function(selectionValue){
        ...
        if(layerString.indexOf('basicgov/') != -1){
          this._clearBGSearchLayer();
          this.search(g.geometry, this.currentLayerIndex, layerString, null, true);
