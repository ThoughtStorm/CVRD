...
define([
  ...
  function(
    declare, _WidgetsInTemplateMixin, BaseWidget,TabContainer, List, Message, utils, tokenUtils, Query, QueryTask, GraphicsLayer, FeatureLayer, esriConfig,
    Graphic, esriUnits, GeometryService, BufferParameters, Point, Circle, SimpleMarkerSymbol, PictureMarkerSymbol, Polyline, SimpleLineSymbol, Polygon, SimpleFillSymbol, Draw, InfoTemplate,
    esriRequest, geometryEngine, ProgressBar, lang, on, html, array, all, Select, TextBox, DrawBox,LoadingShelter, Message, Color) {/*jshint unused: false*/
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
      name: 'Search',
      baseClass: 'jimu-widget-search2',
      resultLayers:[],
      layerIndex1: 0,
      layerIndex2: 0,
      progressBar: null,
      ...
      _initDrawBox:function(){
        ...
        this.own(on(this.drawBox,'DrawEnd',lang.hitch(this,function(graphic){
		  ...
		  //if buffer is selected use buffer geometry
		  if(this.searchDrawBufferValue.value !== null ) {
			  if(this.searchDrawBufferValue.value > 0) {
				var bufferedGeometries = geometryEngine.buffer(graphic.geometry, [this.searchDrawBufferValue.value], this.getEsriSRUnitType(this.searchDrawBufferUnit.value), true);
				this.search(bufferedGeometries, this.layerIndex1, this.selectLayer1.get('value'), null, null);	
			  }	
			else {
				//search without buffer
	  ...
	  _checkTokenAuth: function(){
	  	if(this.appConfig.tokenInfo != null && this.appConfig.tokenInfo.tokenServicesUrl != '' && this.appConfig.tokenInfo.serviceUrl != ''){
	  		var cred = tokenUtils.getCredential(this.appConfig.portalUrl);
	  		if(cred != null)
	    		return cred.token;
	    	else return '';
	  	}
	  ...
	  /** get the icon for the features based icon path specified for the layer in config.json **/
	  ...
	  _drawBGResults: function(results){
		...
		for(var i = 1; i < results.length; i++){
			...
			"<br> <button id='btnGo' onClick='var selectList = dojo.byId(\"bgSearchInfoSelect\"); var pickValue = selectList.value; var bufferValueSelect = dojo.byId(\"bufferRadiusValue\"); var bufferValue = bufferValueSelect.value; var bufferUnitSelectList = dojo.byId(\"bufferRadiusUnitSelect\"); var bufferUnit = bufferUnitSelectList.value; dojo.publish(\"searchWidgetEvent\", {pickValue, bufferValue, bufferUnit});' dojoType='dijit/form/Button'>Go</button> ";
	  ...
	  bufferSearchInSearchWidget:function(selectionValue){
		...
		if(layerString.indexOf('basicgov/') != -1){
			this._clearBGSearchLayer();
			this.search(g.geometry, null, layerString, null, true);
		...
		// GIS search
			...
			this.search(g.geometry, layerIndex, layerString, null ,true);
		}
	  },
			...
			for (var i=0; i<features.length; i++) {
				...
				if(bgResult!=null && bgResult.length>0){		
					if(bgResult[0].obj[bgFieldName] != null && feature.attributes[GISFieldName] == bgResult[0].obj[bgFieldName]){		
							feature.infoTemplate.content += "<br/>";		
						for( var key in bgResult[0].actionLinks) {		
							feature.infoTemplate.content += "<a href=\"" + bgResult[0].actionLinks[key]+"\" target=\"_blank\">" + key+ "</a> <br/><br/>";		
						}		
					} 		
				}
			}
      ...
      _drawResults: function(layerIndex, results) {
        ...
        this._addLinksGISResults(layerIndex, results, features);
        for (var i = 0, len = features.length; i < len; i++) {
          ...
          if(!feature.infoWindow){
			...
			content += "<br/>";
			for( var key in feature.actionLinks) {
				content += "<a href=\"" + feature.actionLinks[key]+"\" target=\"_blank\">" + key+ "</a> <br/><br/>";
			}
	  ...
	  _initUnitSelect:function(){
		...
		//set draw search units
		this.searchDrawBufferUnit.addOption(options);
		this.searchDrawBufferUnit.options[0].selected = false;
		this.searchDrawBufferUnit.options[3].selected = true;
	  },
	  ...
	  //get esri units from config for geometryEngine.buffer()
	  getEsriSRUnitType:function(unit) {
		for(var i = 0; i < this.config.esriSRUnitType.length; i++){
          var unitInfo = this.config.esriSRUnitType[i];
