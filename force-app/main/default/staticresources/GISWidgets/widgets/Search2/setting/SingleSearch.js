...
function(declare, lang, array, html, query, on,json,_WidgetBase,_TemplatedMixin, _WidgetsInTemplateMixin,
  Tooltip,template,TextBox,LayerFieldChooser,IncludeButton,SimpleTable,URLInput,esriRequest, Select) {/*jshint unused: false*/
  return declare([_WidgetBase,_TemplatedMixin,_WidgetsInTemplateMixin], {
    ...
    setConfig:function(config){
	  ...
	  if(this.layerType.get('value') == 'feature'){
		html.setStyle(this.btnBrowse,'visibility','visible');
	  } else {
		html.setStyle(this.btnBrowse,'visibility','hidden');
	  }
      var displayFields = this.config.fields.field;
      this._addDisplayFields(displayFields, this.config.titlefield);
      //BG may not need to do this if is basicgov layer
	  if(this.layerType.get('value') == 'feature'){
    ...
    _bindEvents:function(){
	  ...
	  //BG added
	  this.own(on(this.layerType,'change',lang.hitch(this,function(){
	    console.debug('*** Yes, I am in.');
		if(this.layerType.get('value') == 'feature'){
			html.setStyle(this.btnBrowse,'visibility','visible');
		} else{
			html.setStyle(this.btnBrowse,'visibility','hidden');
    ...
    validate:function(showTooltip){
	  ...
	  //BG if is BasicGov Layer, do not do this validation
	  if(this.layerType.get('value') == 'feature'){
