...
define([
  ...
  function(
    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
	  ...
	  _addLinksGISResults: function(layerIndex, results, feature) {
	  	var layerConfig = this.config.layers[layerIndex];
			...
			for(var j = 0; j<layerFields.length; j++){
				if(layerFields[j].bgField != null){
					...
					var featureAttributes = feature.attributes;
      				for (var att in featureAttributes) {
      					if(att==layerFields[j].name){
      						bgFieldValue = featureAttributes[att];
      						break;
      					}
      				}
      				if(bgFieldValue != null && bgFieldValue != ""){
      					this.bgManager.getBGLinks(bgFieldValue, bgFieldName, layerConfig.bgObject, layerConfig.name);
      					dojo.subscribe('bgLinkResults_'+layerConfig.name+'_'+layerConfig.bgObject, lang.hitch(this, this._addBGLinks, layerIndex, feature));
						break;
					}
          		}
			}
	  ...
	  // Async method that captures the action links and places them into the Details Tooltip for GIS Shape search
	  _addBGLinks(layerIndex, feature, bgResult) {
	  	if(bgResult!=null && bgResult.length>1){
	  		var layerConfig = this.config.layers[layerIndex];
	  		var layerFields = layerConfig.fields.field;
			...
			if(bgResult[1].obj[bgFieldName] != null && feature.attributes[GISFieldName] == bgResult[1].obj[bgFieldName]){
		  		feature.infoTemplate.content += "<br/>";
				for( var key in bgResult[1].actionLinks) {
					feature.infoTemplate.content += "<a href=\"" + bgResult[1].actionLinks[key]+"\" target=\"_blank\">" + key+ "</a> <br/><br/>";
				}
			}
		}
		this.shelter.hide();
      ...
      _drawResults: function(layerIndex, results) {
        ...
        for (var i = 0, len = features.length; i < len; i++) {
          ...
          if(feature.actionLinks == null){
          	...
          	this._addLinksGISResults(layerIndex, results, feature);
          }
