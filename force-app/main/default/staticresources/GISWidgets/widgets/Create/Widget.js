...
define([
  ...
  function(declare, lang, array, html, esriBundle, on, query, Json, Deferred, aspect, all,
	...
	domConstruct, Draw, Graphic, InfoTemplate, WebMercatorUtils, domStyle, Message) {
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
	  ...
	  bgManager: null,
	  bgTemplatePicker: null,
	  bgGeoFieldNames: null,
      ...
      startup: function() {
        ...
        this.editPopup = new Popup(null, html.create("div",
                                                    ...
                                                    null,
                                                    this.map.root));
		this.bgEditPopup = new Popup(null, html.create("div",
                                                    {"class":"jimu-widget-edit-infoWindow"},
                                                    null,
                                                    this.map.root));
      ...
      _getTemplatePicker: function(layerInfos) {
		...
		var heightStyle = "height: 40%";
		var templatePicker = new TemplatePicker({
          featureLayers: this._layerObjectsParaForTempaltePicker,
          grouping: true,
          rows: "auto",
          columns: "auto",
          style: bottomStyle + ";" + topStyle + ";" + heightStyle
      ...
      _createEditor: function() {
		...
		if(params.settings.basicgovLayerInfos)
		{
			this._createDomForBGLayers();
		}
      },
	  
	  /***************************************
      * Methods for BG works
      ****************************************/
	  
	  //BG method - Creates all the doms needed for the BG Layer in the Edit Widget
	  _createDomForBGLayers: function() {
		  var bgLayerInfosParam = new Map();
		  array.forEach(this._configEditor.basicgovLayerInfos, function(bgLayerInfo) {
			var layerObjects = this.map._layers;
			var objectName = bgLayerInfo.basicgovLayer.id;
			Object.keys(layerObjects).map(function(key, index) {
				if(key.indexOf(objectName) > -1 && !bgLayerInfosParam.has(objectName))
				{
				  bgLayerInfo.basicgovLayer = layerObjects[key];
				  bgLayerInfosParam.set(objectName,bgLayerInfo);
				}
			}, this);
		}, this);
		
		//create bgTemplatePicker only if there are BG Layers visible on the map using LayerList widget
		if(bgLayerInfosParam.size > 0)
		{
			var templatePickerItems = [];
			var allObjNames = [];
			
			bgLayerInfosParam.forEach(function (bgLayerInfoItem, objectNameKey, mapObj) {  
				var objectName = objectNameKey;	
				var objectLabel = bgLayerInfoItem.label;
				
				var item = {
					label: "New " + objectLabel,
					symbol: bgLayerInfoItem.basicgovLayer.graphics[0].symbol,
					description: objectName
				};
				templatePickerItems.push(item);
				allObjNames.push(objectName);
			});  
			
			if (isLightning) {
			  // need to get the geo field names only for LEX
			  this.bgManager = new BGManager();
			  this.bgManager.getGeoFieldNames(allObjNames);
			  dojo.subscribe('editWidgetGetGeoFieldNames', lang.hitch(this, this._handleGeoFieldNames));
		    }
	  
			//Template creation	for BG Layers	
			var bottomStyle = this._configEditor.toolbarVisible ? "" : "bottom: 0px";
			var topStyle = this._configEditor.useFilterEdit ? "top: 65%" : "top: 40%";
					
			var bgTemplatePicker = new TemplatePicker({
			...
			bgTemplatePicker.startup();
			this.bgTemplatePicker = bgTemplatePicker;
			
			var bgTemplatePickerLabel = domConstruct.create('div', {
			'innerHTML': 'Clariti Layers',
			'id': 'bgTemplatePickerLabel'
			}, this.domNode);
			domStyle.set(bgTemplatePickerLabel, 'top', '62%');
      domStyle.set(bgTemplatePickerLabel, 'position', 'absolute');
			
			var drawToolbar = new Draw(this.map);

            var selectedTemplate;
            bgTemplatePicker.on("selection-change", function() {
              if( bgTemplatePicker.getSelected() ) {
                selectedTemplate = bgTemplatePicker.getSelected();
              }
              drawToolbar.activate(Draw["POINT"]);
            });
			
			// storing the this object in a variable so that we can use it in the "draw-end" event handler
			var thisWidget = this;
		  
		    drawToolbar.on("draw-end", function(evt) {
              drawToolbar.deactivate();
			  
			  //deselect the selected template in the BG TemplatePicker once drawing has ended
			  if(document.getElementsByClassName("selectedItem").length > 0 && document.getElementsByClassName("selectedItem")[0].classList)
				document.getElementsByClassName("selectedItem")[0].classList.remove("selectedItem");
			  
			  // get the links for create records using VF remote
			  thisWidget._getCreateRecordLink(selectedTemplate, evt);
			});
		}
	  },
	  
	  // BG Method - for Lightning Experience, need field names of the geolocation fields
      _handleGeoFieldNames: function(result){
        var isbgError = this._isBGError(result);
        if (isbgError) {
          return;
        }
        result = result[0];
        this.bgGeoFieldNames = result;
        dojo.subscribe('errorFromGetRecordTypes', lang.hitch(this._handleCreateError));
      },
	  _handleCreateError: function(result){
        new Message({
          message: result
        });
      },
	  // BG Method - to make the call to apex to get Create Link for the object with the geolocation fields pre-filled
	  _getCreateRecordLink: function(selectedTemplate, evt){
		if (isLightning) {
			var objName = selectedTemplate.item.description;
			var latField = this.bgGeoFieldNames[objName][0];
			var lonField = this.bgGeoFieldNames[objName][1];
			var degreeDecimalVal = WebMercatorUtils.xyToLngLat(evt.geometry.x, evt.geometry.y);
			var latVal = degreeDecimalVal[1];
			var lonVal = degreeDecimalVal[0];
			var defaultVals = {};
			defaultVals['Name'] = 'System Number';
			defaultVals[latField] = latVal;
			defaultVals[lonField] = lonVal;
							        
			var params = {
			defaultValues: defaultVals,
			objectType: objName
			};
			this._addBGCreateRecordLink(selectedTemplate, evt, params);
		} else {
			// wrapper object to contain all the parameters for the bgManager.getBGCreateRecordLink function		
			...
			var degreeDecimalVal = WebMercatorUtils.xyToLngLat(evt.geometry.x, evt.geometry.y);
			var fieldVals = [degreeDecimalVal[1], degreeDecimalVal[0]]; //[Latitude, Longitude]
			var param = new LinkParamVO(fieldVals, null, selectedTemplate.item.description, selectedTemplate.item.label.substring(3));		
			this.bgManager = new BGManager();
			this.bgManager.getBGCreateRecordLink(param);
			  
			dojo.subscribe('bgCreateRecordLinkResults_'+selectedTemplate.item.description, lang.hitch(this, this._addBGCreateRecordLink, selectedTemplate, evt));
		}
	  },
	  
	  // BG Method -  to add the Create Link to the Info template or show error message if there is an exception thrown in apex
	  _addBGCreateRecordLink: function(selectedTemplate, evt, result){
		  var bgError = (isLightning)? false : this._isBGError(result);
		  if(bgError == true)
		  {
			  return;
		  }
		  var createLink = result[1];
		  var degreeDecimalVal = WebMercatorUtils.xyToLngLat(evt.geometry.x, evt.geometry.y);
		  var attr = {"Xcoord":degreeDecimalVal[0],"Ycoord":degreeDecimalVal[1]};
		  var templateString = "Latitude: ${Ycoord} <br/> Longitude: ${Xcoord}";
		  if (isLightning) {
			templateString += '<br/><a href=\'javascript:void(0);\' onclick=\'getRecordTypes(' + JSON.stringify(result) + ');\'>Create ' + selectedTemplate.item.label.substring(3) + '</a><br/><br/>';
		  } else {
			  var createLink = result[1];
			  templateString += "<br/> <a href=\"" + createLink +"\">Create " + selectedTemplate.item.label.substring(3) + "</a> <br/><br/>";
		  }
		  var infoTemplate = new InfoTemplate(selectedTemplate.item.label,templateString);
		  
		  var graphic = new Graphic(evt.geometry, selectedTemplate.item.symbol, attr, infoTemplate);
		  this.map.graphics.clear();
		  this.map.graphics.add(graphic);
		  
		  var point = graphic.geometry;
		  this.map.setInfoWindow(this.bgEditPopup);
		  this.bgEditPopup.setFeatures([graphic]);
		  // keep it consistent with the info template in other widgets
		  this.bgEditPopup.resize(270, 200);
		  this.bgEditPopup.show(point);
		  
		  // event handler for when info window is closed for BG record creation
		  this.own(on(this.bgEditPopup,
                    'hide',
                    lang.hitch(this, this._onBGEditorPopupHide)));
	  },
	  
	  // BG Method - to check if there is an error then display error message
      ...
      ****************************************/
      ...
      _worksAfterClose: function() {
		...
		// destroy bgTemplatePickerLabel
		if(document.getElementById('bgTemplatePickerLabel')) {
			domConstruct.destroy("bgTemplatePickerLabel");
		}
		// destroy bgTemplatePicker
		if(this.bgTemplatePicker) {
			this.bgTemplatePicker.destroy();
		}
	  ...
	  _onBGEditorPopupHide: function() {
		this.bgEditPopup.clearFeatures(); 
		this.map.graphics.clear();
		this.map.setInfoWindow(this.editPopup);
	  }
